...
from model_zero import generate_model, train_model, test_model
import sys
import os
...
if __name__ == '__main__':
    ...
    model_zero = generate_model(shape=(1+n_gate,1000,n_gate),
                           sparsity=0.5, radius=0.1, scaling=(1.0,1.0),
                           leak=1.0, noise=(0.0000, 0.000, 0.000))
    ...
    # Train model
    error = train_model(model_zero, train_data)
    print("Training error : {0}".format(error))

    # Test model
    error = test_model(model_zero, test_data)
    ...
    ax1.plot(model_zero["output"], color='0.00', lw=1.5)
